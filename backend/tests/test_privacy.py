...
from privacy.models import ConsentRecord, AuditLog, DataExportJob
import privacy.views as pv  # para parchear .delay a helpers síncronos
...
def test_privacy_flow_consent_export_erase(db, monkeypatch):
    ...
    assert ConsentRecord.objects.filter(
        user__email="alumno@example.com", institution=inst, kind="privacy"
    ).exists()
    assert AuditLog.objects.filter(
        action="privacy.consent.create", institution=inst
    ).exists()
    ...
    # --- 2) Export ---
    # Parchea .delay para que invoque el helper síncrono (django-rq no tiene .func)
    monkeypatch.setattr(
        pv.run_export_job, "delay",
        lambda job_id: pv.run_export_job_sync(job_id)
    )
    ...
    assert detail.data["result_url"]  # url del .json generado

    assert AuditLog.objects.filter(
        action="privacy.export.request", institution=inst
    ).exists()

    # --- 3) Erase ---
    # Asegura que exista al menos una DeviceSession por borrar
    assert DeviceSession.objects.filter(
        user__email="alumno@example.com", institution=inst
    ).exists()

    monkeypatch.setattr(
        pv.run_erase_job, "delay",
        lambda job_id: pv.run_erase_job_sync(job_id)
    )

    erq = c.post(f"{BASE}/privacy/erase", {"confirm": "ERASE"}, format="json")
    assert erq.status_code == 201, erq.content
    assert AuditLog.objects.filter(
        action="privacy.erase.request", institution=inst
    ).exists()

    # La tarea ya corrió; sesiones borradas y enrollment desactivado
    assert not DeviceSession.objects.filter(
        user__email="alumno@example.com", institution=inst
    ).exists()
    enr = Enrollment.objects.get(
        user__email="alumno@example.com", institution=inst
    )
